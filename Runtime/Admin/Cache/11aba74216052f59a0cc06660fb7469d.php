...
<div class="pageContent">
	<form method="post" action="__URL__/update/navTabId/__MODULE__" class="pageForm required-validate" enctype="multipart/form-data" onsubmit="return iframeCallback(this, dialogAjaxDone)">
		...
		<div class="pageFormContent" layoutH="58">
			...
			<div class="unit">
				<label>别名：</label>
				<input type="text" minlength="2" name="b_name" value=<?php echo ($vo["b_name"]); ?>>
			</div>
			
			<div class="unit">
				<label>会员：</label>
				<input type="hidden" name="uid" group="member" field="id" value="<?php echo ($vo["uid"]); ?>">
				<input class="required" name="member_name" type="text" readonly="readonly" group="member" field="name" value="<?php echo (getParent('Member',$vo["uid"])); ?>"/>
			...
			<div class="unit">
				<label>企业名称：</label>
				<input type="text" name="companyname" value="<?php echo ($vo["companyname"]); ?>">
			</div>
			
			<div class="unit">
				<label>法人代表：</label>
				<input type="text" name="legal" value="<?php echo ($vo["legal"]); ?>">
			</div>
			
			<div class="unit">
				<label>营业执照有效期：</label>
				<input type="text" name="validity" class="date" readonly="true" format="yyyy-MM-dd" value="<?php echo ($vo["validity"]); ?>" />
				<a class="inputDateButton" href="javascript:;">选择</a>
			</div>
			
			<div class="unit">
				<label>营业执照：</label>
				<input type="hidden" name="license" id="license" value="<?php echo ($vo["license"]); ?>">
				<div style="float: left;">
				<?php if(!empty($vo["license"])): ?><img id="license_img" width="100" height="100" src="__ROOT__<?php echo ($vo["license"]); ?>" style="margin-left:5px;"/>
                <?php else: ?>
				<img id="license_img" width="100" height="100" style="margin-left:5px; display:none;"/><?php endif; ?>
			...
			<div class="unit">
				<label>负责人姓名：</label>
				<input type="text" name="fz_name" value="<?php echo ($vo["fz_name"]); ?>">
			</div>
			<div class="unit">
				<label>负责人地址：</label>
				<input type="text" name="fz_address" value="<?php echo ($vo["fz_address"]); ?>">
			</div>
			<div class="unit">
				<label>负责人电话：</label>
				<input type="text" name="fz_tel" value="<?php echo ($vo["fz_tel"]); ?>">
			</div>
			<div class="unit">
				<label>负责人手机：</label>
				<input type="text" name="fz_phone" value="<?php echo ($vo["fz_phone"]); ?>">
			</div>
			<div class="unit">
				<label>负责人邮箱：</label>
				<input type="text" name="fz_mail" value="<?php echo ($vo["fz_mail"]); ?>">
			</div>
			<div class="unit">
				<label>传真：</label>
				<input type="text" name="fax" value="<?php echo ($vo["fax"]); ?>">
			</div>
			
			<div class="unit">
				<label>预约电话：</label>
				<input type="text" class="tel" name="tel" value="<?php echo ($vo["tel"]); ?>">
			</div>

            <div class="unit">
                <label>营业时间：</label>
                <input type="text" name="opening" value="<?php echo ($vo["opening"]); ?>">
            </div>
			
			<div class="unit">
				<label>商家类型：</label>
				<input type="text" class="type" name="type" value="<?php echo ($vo["type"]); ?>">
			</div>
			<div class="unit">
				<label>商家特色：</label>
				<input type="text" class="characteristic" name="characteristic" value="<?php echo ($vo["characteristic"]); ?>">
			</div>

            <div class="unit">
                <label>服务范围：</label>
                <input type="text" class="services" name="services" value="<?php echo ($vo["services"]); ?>">
            </div>
			
			<div class="unit">
				<label>商家自我签名：</label>
				<input type="text" class="textInput" name="signature" id="signature" style="width:330px;" value="<?php echo ($vo["signature"]); ?>">
			</div>
			...
			<div class="unit">
				...
				<SELECT name="isbrand" class="combox">
					<option value="0" <?php if(($vo["isbrand"])  ==  "0"): ?>selected<?php endif; ?>>否</option>
					<option value="1" <?php if(($vo["isbrand"])  ==  "1"): ?>selected<?php endif; ?>>是</option>
				</SELECT>
			</div>
			...
			<div class="unit">
				...
				<SELECT name="real_scene" class="combox">
					<option value="0" <?php if(($vo["real_scene"])  ==  "0"): ?>selected<?php endif; ?>>否</option>
					<option value="1" <?php if(($vo["real_scene"])  ==  "1"): ?>selected<?php endif; ?>>是</option>
				</SELECT>
			</div>
            
            <div class="unit">
				<label>资质认证：</label>
                <label class="radioButton"><input type="radio" name="qualifications" value="0" <?php if(($vo["qualifications"])  ==  "0"): ?>checked="checked"<?php endif; ?>>未验证</label>
                <label class="radioButton"><input type="radio" name="qualifications" value="1" <?php if(($vo["qualifications"])  ==  "1"): ?>checked="checked"<?php endif; ?>>已验证</label>
			</div>
            
            <div class="unit">
				<label>保证金：</label>
                <label class="radioButton"><input type="radio" name="security" value="0" <?php if(($vo["security"])  ==  "0"): ?>checked="checked"<?php endif; ?>>未验证</label>
                <label class="radioButton"><input type="radio" name="security" value="1" <?php if(($vo["security"])  ==  "1"): ?>checked="checked"<?php endif; ?>>已验证</label>
			</div>
			
            <div class="unit">
				<label>入驻时间：</label>
				<input type="text" name="addtime" class="date" readonly="true" format="yyyy-MM-dd HH:mm:ss" value="<?php echo ($vo["addtime"]); ?>" />
				<a class="inputDateButton" href="javascript:;">选择</a>
			</div>
