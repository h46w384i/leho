	...
	$(".__MODULE__Tree li a").contextMenu('__MODULE__TreeCM', {
	      ...
	      bindings:{
	    	  	...
	    	  	indexData:function(t){
	    	  		var rid = t.attr('rid');
	            	var url = APP+'/Goods/index/rid/'+rid;
	            	var tabid = 'Goods';
	            ...
	            deleteList:function(t){
	            	var rid = t.attr('rid');
					var url = APP+'/Goods/clearList/rid/'+rid;
					var title='确定要清空吗？';
	            ...
	            addData:function(t){
	            	var rid = t.attr('rid');
	            	var url = APP+'/__MODULE__/add/id/'+rid;
	            ...
	            editData:function(t){
					var rid = t.attr('rid');
					var url = APP+'/__MODULE__/edit/id/'+rid;
	            ...
	            deleteData:function(t){
	            	var rid = t.attr('rid');
					var url = APP+'/__MODULE__/foreverdelete/id/'+rid;
					var title=' 确定要删除吗？';
					alertMsg.confirm(title, {
	                        okCall: function(){
	                              ajaxTodo(url,function(data){
...
<div style="float:left; display:block; overflow:auto; width:100%; border:solid 1px #CCC; line-height:21px; background:#fff">
    ...
    <li><a href="javascript:;" rid="0">无</a>
    <ul>
    <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><?php $before = intval($data[$key-1]['level']);
		...
		<li><a href="javascript:;" rid="<?php echo ($vo['id']); ?>"><?php echo ($vo["name"]); ?></a>
