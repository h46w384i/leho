...
function contentFilter($content){
	...
	$content = labelFliter($content);
	$content = replacestr($content);
	return $content;
}

function labelFliter($content){
	preg_match_all('/#([^#]+)#/i', $content, $arr);
	if(!empty($arr[1])){
		$label = D('Label');
		foreach ($arr[1] as $key => $vo){
			$map['name'] = array('eq',$vo);
			$labeldata = $label->where($map)->find();
			if(!empty($labeldata)){
				$str = '<a href="'.U('Circle/index/lid/'.$labeldata['id']).'">#'.$vo.'#</a> ';
...
function MakeGoodMap($id=0,$goodinfo=array(),$reMk = false){
	...
	if(!empty($goodinfo['latitude'])&&!empty($goodinfo['longitude'])){
		...
		if(!file_exists($file_name)|| $reMk ==true){
		    import("ORG.Net.Http");
			/*if(C('sysconfig.site_map_isgoogle')==1){
				$str = Http::fsockopen_download("http://maps.google.com/maps/api/staticmap?center=".$goodinfo['latitude'].",".$goodinfo['longitude']."&zoom=14&size=200x200&maptype=mobile&markers=".$goodinfo['latitude'].",".$goodinfo['longitude']."&sensor=false");
			}else{*/
...
function getIpLatlng($ip = ''){
	...
	/*if(C('sysconfig.site_map_isgoogle')==1){
		$url = 'http://maps.googleapis.com/maps/api/geocode/json?address='.$address.'&sensor=true&language='.C('DEFAULT_LANG');
		$json = Http::fsockopen_download($url,$conf);
		$arr = json_decode($json,true);
		if($arr['status'] == 'OK'){
			$latlng = $arr['results'][0]['geometry']['location'];
