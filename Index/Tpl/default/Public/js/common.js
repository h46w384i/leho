...
jQuery.fn.qqFace = function(contentBox,callback,child) {
	var box = $(contentBox);
	...
	return this.each(function(){
		var t = $(this);
		if(child){
			box = t.parents(contentBox).find(child);
		}
		t.unbind('click');
		t.bind('click',function(){
			clearEvent();
		    var offset = t.offset();
			var xtop = offset.top + t.height();
...
jQuery.fn.tip = function(content,time) {
    return this.each(function(){
    	...
    	var html = '<div class="jvf_prompt" id="'+uuid+'" '+style+'>'+content+'<span class="jvf_allimg"></span></div>';
    	$('body').append(html);
    	if(time){
...
function globalEvent(){
	...
	$('.information_box_head .but_a[add="add"],.guanzhu').live('click',function(){
		var t = $(this);
	...
	$('.person .top-user').hover(function(){
		$(this).addClass('hover');
	},function(){
		$(this).removeClass('hover');
	});
	
	$('.person .top-mess').hover(function(){
		$(this).addClass('hover');
	},function(){
		$(this).removeClass('hover');
	});
...
function index(){
	$('.switch-scroll-next').click(function(){
		var lis = $('.switch-nav li');
		var num = lis.length - 1;
		var selected = $('.switch-nav li.selected');
		var index = lis.index(selected);
		if(index == num){
			$(this).removeClass('switch-scroll').addClass('switch-scrolled');
		}else{
			$('.switch-scroll-prev').removeClass('switch-scrolled').addClass('switch-scroll');
			lis.attr('class','unselected');
			lis.eq(index+1).attr('class','selected');
			if(index+1 == num){
				$(this).removeClass('switch-scroll').addClass('switch-scrolled');
			}
		}
	});
	$('.switch-scroll-prev').click(function(){
		var lis = $('.switch-nav li');
		var num = lis.length - 1;
		var selected = $('.switch-nav li.selected');
		var index = lis.index(selected);
		if(index == 0){
			...
			$('.switch-scroll-next').removeClass('switch-scrolled').addClass('switch-scroll');
			lis.attr('class','unselected');
			lis.eq(index-1).attr('class','selected');
			if(index-1 == 0){
				$(this).removeClass('switch-scroll').addClass('switch-scrolled');
...
function jvfDialogHtml(id,html){
	if($('#'+id).html()){
		$('#'+id).dialog('open');
	}else{
...
function upAlbum(aid){
	loginAfter(function(){
		jvfDialog('sharebox', APP+'/Member/sharebox/aid/'+aid);
	});
}
...
function city(){
			...
			$.getJSON(APP+'/Index/cityChild/pid/'+pid,function(data){
				if(data.status == 1){
					...
					for(var vo in data.info){
						html += '<option value="'+data.info[vo].id+'">'+data.info[vo].name+'</option>';
					}
	...
	$('#changeCity input[type="button"]').click(function(){
		...
		if(cy != -1){
			goUrl(APP+'/Index/switchRegion/id/'+cy);
		}
	});
...
function signin(){
	var submit = function(){
		...
		var signin_email = $('#signinform input[name="email"]').val();
		var signin_password =$('#signinform input[name="password"]').val();
		if(!signin_email){
			$('#signinform input[name="email"]').tip(L.PLEASE_INPUT_MAIL);
		}else if(!signin_password){
			$('#signinform input[name="password"]').tip(L.PLEASE_INPUT_PASSWORD);
		}else{
			$.post(action,para,function(data){
				t.tip(data.info,2000);
				if(data.status==2)goUrl(APP+"/User/noverifymail");
				if(data.status==1){
					var url = $.cookie('before_login');
					if(url){
						$.cookie('before_login',null);
					}else{
						url = APP+'/Member/index';
					}
					goUrl(url);
				}
			},'json');
    ...
    $('#signinform #password').keydown(function(e){
        if(e.keyCode == 13){
            submit();
        }
    });
...
function register(){
	$('#user_new #submit').click(function(){
		...
		var t = $(this);
		var user_first_name = $('#user_new input[name="name"]').val();
		var user_email =$('#user_new input[name="email"]').val();
		var user_password =$('#user_new input[name="password"]').val();
		var user_password_confirmation =$('#user_new input[name="password_confirmation"]').val();
		if(!user_first_name){
			$('#user_new input[name="name"]').tip(L.PLEASE_INPUT_USERNAME);
		}else if(!user_email){
			$('#user_new input[name="email"]').tip(L.PLEASE_INPUT_MAIL);
		}else if(!user_password){
			$('#user_new input[name="password"]').tip(L.PLEASE_INPUT_PASSWORD);
		}else if(!user_password_confirmation){
			$('#user_new input[name="password_confirmation"]').tip(L.PLEASE_INPUT_CONFIRM_PASSWORD);
		}else{
			$.post(action,para,function(data){
				t.tip(data.info,2000);
...
function goodsIndex(gid){
	...
	$('#businessesMap').click(function(){
		...
		jvfDialog('bmaps', APP+'/Index/maps/bid/'+bid);
	});
...
function talk_about(){
	var countNum = function(){
		...
		var html;
		if(textNum < 0){
			html = '<div class="count-txt error">超出<span class="txt">'+Math.abs(textNum)+'</span>字</div>';
			$('#talk_aboutBox #submit').removeClass('btn-t1').addClass('btn-default-32');
		}else{
			html = '<div class="count-txt">还可以输入<span class="txt">'+textNum+'</span>字</div>';
			if(num != 0){
				$('#talk_aboutBox #submit').removeClass('btn-default-32').addClass('btn-t1');
			}else{
				$('#talk_aboutBox #submit').removeClass('btn-t1').addClass('btn-default-32');
			}
		}
	...
	$('#talk_aboutBox input[name="consume_date"]').datepicker({changeMonth: true,changeYear: true,shortYearCutoff: 50,maxDate:new Date(),hideIfNoPrevNext:true});
	...
	$('#talk_aboutBox input[name="price"]').focus(function(){
		$(this).removeTip();
	});
	
	$('#talk_aboutBox .consume_showin_timeline').click(function(){
		var t = $(this);
		var consume_showTimeline = $('#talk_aboutBox input[name="consume_showTimeline"]');
		if(t.is('.checked')){
			t.removeClass('checked').addClass('checkbox');
			consume_showTimeline.val(0);
		}else{
			t.removeClass('checkbox').addClass('checked');
			consume_showTimeline.val(1);
		}
	});
	...
	$('#talk_aboutBox #submit').click(function(){
		...
		loginAfter(function(){
			...
			var num = getLength($('#talk_aboutBox textarea[name="content"]').val());
			var imgsNum = $('#talk_aboutBox .up-pic').length;
			if(num == 0 && imgsNum <= 1){
				return false;
			}
			var price = $('#talk_aboutBox input[name="price"]').val();
			if(price){
				var re = /^[\d]+[\.]{0,1}[\d]{0,2}$/;
				if(!re.test(price)){
					$('#talk_aboutBox input[name="price"]').tip('请输入阿拉伯数字，如149.99');
					return false;
				}
			}
			var textNum = maxTextNum - num;
			if(textNum < 0){
				...
				$.post(APP+'/Member/doTalk_about',para,function(data){
					if(data.status == 0){
						...
						jalert(L.RELEASE_SUCCESS);
						$('#talk_aboutBox .jvf_sample_list').html('');
						$('#talk_aboutBox textarea[name="content"]').val('');
						$('#talk_aboutBox input[name="price"]').val('');
						$('#talk_aboutBox input[name="place"]').val('');
						$('#talk_aboutBox #img_list .re-up-pic').prevAll().remove();
						modifyimgs();
						countNum();
	...
	$('#talk_aboutBox .jvf_sample a.del').live('click',function(){
		...
		new AjaxUpload('#imgs', {
		    ...
		    onComplete : function(file,response){
		    	...
		    	if(data.err == ''){
		    		var html='<div class="up-pic"><div class="pic"><img src="'+data.msg.thumbnail+'"><a href="#" onclick="return false" class="del"></a></div><input name="imgs[]" type="hidden" value="'+data.msg.id+'"><input placeholder="为图片说点什么吧" type="text" value="" class="txt" name="alt[]"></div>';
		    		$('#talk_aboutBox #imgs').parent().before(html);
		    	}else{
		    		$('#talk_aboutBox #imgs').tip(data.err);
		    	}
		    	modifyimgs();
		        this.enable();
		    }
		});
		$('#talk_aboutBox #imgs').click(function(){
			$('input[name="images"]').trigger('click');
		});

	function modifyimgs(){
		if($('#talk_aboutBox .up-pic').length > 1){
			$('#talk_aboutBox #submit').removeClass('btn-default-32').addClass('btn-t1');
			$('#talk_aboutBox #imgs').addClass('has');
		}else{
			var num = getLength($('#talk_aboutBox textarea[name="content"]').val());
			if(num <=0){
				$('#talk_aboutBox #submit').removeClass('btn-t1').addClass('btn-default-32');
			...
			$('#talk_aboutBox #imgs').removeClass('has');
		}
	}
	
	$('#talk_aboutBox .up-pic .del').live('click',function(){
		var t= $(this);
		t.parents('.up-pic').remove();
		modifyimgs();
	});
}
...
function shoppingCart(){
	...
	$('#cartSelectAllTop,#cartSelectAllBot').click(function(){
		...
		var checkbox = $('#shoppingCartList input[type="checkbox"]');
		checkbox.attr('checked',checked);
		if(checked){
			$('#shoppingCartList .checker span').addClass('checked');
		}else{
			$('#shoppingCartList .checker span').removeClass('checked');
		}
	});
	...
	$('a[id*="deleteGoods_"]').click(function(){
		...
		var tbody = $(this).parents('tr');
		tbody.remove();
		statistics();
		var html = $('#shoppingCartList tbody').length;
		if(length < 1){
			$('#haveGoods').hide();
			$('#emptyGoods').show();
		}
...
function circleIndex(condition){
	...
	$('#more-btn').click(function(){
		...
		var uuid = guid();
		var html = '<div class="discover-panel" title="提示">'
	+'<div class="panel-content">'
		...
		+'</div>'
		+'<div class="ft">'
			+'<a id="know-btn" class="btns btn-t1" href="javascript:;" onclick="$(this).parents(\'.ui-dialog\').remove();"><span><strong>知道了</strong></span></a>'
		+'</div>'
	+'</div>'
+'</div>';
	...
	$('.switch-scroll-active .switch-nav li').mouseover(function(){
		var t = $(this);
		var li = $('.switch-scroll-active .switch-nav li');
		var index = li.index(t);
		var content = $('.switch-content li');
		var crr = content.eq(index);
		content.hide();
		li.removeClass('selected');
		t.addClass('selected');
		crr.show();
	...
	$.getScript(PUBLIC+'/dwz/js/jquery.masonry.min.js',function(){
		if(condition.cid){
			var href = APP+'/Circle/ajaxCircle/cid/'+condition.cid;
		}
		if(condition.lid){
			var href = APP+'/Circle/ajaxCircle/lid/'+condition.lid;
		}
		if(condition.aid){
			var href = APP+'/Circle/ajaxCircle/aid/'+condition.aid;
		}
		if(!href){
			var href = APP+'/Circle/ajaxCircle';
		}
		reset(href);
	});
...
function memberTop(){
	$('.myhome-user .user-photo').hover(function(){
		$('#modify_headimg').height(24);
	},function(){
		$('#modify_headimg').height(0);
	});
	
	$('.go-discover').hover(function(){
		$(this).removeClass('go-2');
		$(this).addClass('go-1');
	},function(){
		$(this).removeClass('go-1');
		$(this).addClass('go-2');
	});
	
	$('#medal').hover(function(){
		$(this).find('.me-style').addClass('me-selected');
		$(this).find('.medal-panel').show();
	},function(){
		$(this).find('.me-style').removeClass('me-selected');
		$(this).find('.medal-panel').hide();
	});
	
	$('.block-show .share-btn').click(function(){
		jvfDialog('sharebox', APP+'/Member/sharebox');
	});
	
	$('.medal-list li').hover(function(){
		var t = $(this);
		var content = '<h2>'+t.attr('data-name')+'</h2><p>'+t.attr('data-mark')+'</p>';
		$('#medal_tips').find('.panel-content .bd').html(content);
		$('#medal_tips').show();
	},function(){
		$('#medal_tips').hide();
	}).mousemove(function(e){
		var medal_tips = $('#medal_tips');
		var x = e.pageX - 78;
		var y = e.pageY + 30;
		medal_tips.css('left',x+'px');
		medal_tips.css('top',y+'px');
	});
...
function memberInterest(){
	...
	$('.add-group').click(function(){
		jvfDialog('interestBox', APP+'/Member/interestBox');
	});
	
	$('.tag-block .interest-tag').click(function(){
		var t = $(this);
		var lid = t.attr('lid');
		var uid = t.attr('uid');
		if(uid){
			var href = APP+'/Circle/ajaxCircle/interest/'+uid;
		}else{
			var href = APP+'/Circle/ajaxCircle/lid/'+lid;
		}
		$('#main .column-wraper .jvf_pubu:not(".moudle-block")').remove();
		reset(href);
		var tag = $('.tag-block .interest-tag');
		tag.removeClass('selected');
		t.addClass('selected');
	});
	
	$.getScript(PUBLIC+'/dwz/js/jquery.masonry.min.js',function(){
		var first = $('.tag-block .interest-tag:first');
		var lid = first.attr('lid');
...
function memberPeriphery(){
    memberTop();
    $('.add-group').click(function(){
        jvfDialog('peripheryBox', APP+'/Member/peripheryBox');
    });
    ...
    $('.tag-block .interest-tag').click(function(){
        ...
        var uid = t.attr('uid');
        if(uid){
            var href = APP+'/Circle/ajaxCircle/periphery/'+uid;
        }else{
            var href = APP+'/Circle/ajaxCircle/pid/'+lid;
        }
        $('#main .column-wraper .jvf_pubu:not(".moudle-block")').remove();
        reset(href);
        var tag = $('.tag-block .interest-tag');
...
function peripheryBox(){

    $('.interest-tags .interest-tag').click(function(){
        var t = $(this);
        var lid = t.attr('lid');
        $.getJSON(APP+'/Member/doPeriphery/id/'+lid,function(data){
            if(data.status == 1){
                if(t.is('.selected')){
    ...
    $('.checkbox-btn').click(function(){
        ...
        var a = t.find('dt a');
        if(a.is('.checked')){
            a.removeClass('checked').addClass('checkbox');
            $('.checkbox-wrapper .checkbox-cont a').removeClass('checked').addClass('checkbox');
        }else{
            a.removeClass('checkbox').addClass('checked');
            var bool = true;
            par.find('dt a').each(function(){
                if($(this).is('.checkbox')){
                    bool = false;
                }
            });
            if(bool){
                $('.checkbox-wrapper .checkbox-cont a').removeClass('checkbox').addClass('checked');
            }
        }
    }).hover(function(){
            var t = $(this);
    ...
    $('.btn-follow-20-add').click(function(){
        var t = $(this);
        var id = t.attr('data-district_id');
        $.getJSON(APP+'/Member/doPeriphery/id/'+id,function(data){
            if(data.status == 1){
                t.removeClass('btn-follow-20-add').addClass('btn-follow-20-end');
            }
        });
        return false;
    });

    $('.btn-follow-20-end').click(function(){
        var t = $(this);
        var id = t.attr('data-district_id');
        $.getJSON(APP+'/Member/doPeripheryDel/id/'+id,function(data){
            if(data.status == 1){
                t.removeClass('btn-follow-20-end').addClass('btn-follow-20-add');
            }
        });
        return false;
    });

    $('#add_district_select').click(function(){
        $('.current .btn-follow-20-add').trigger('click');
    });

    $('.global-page a').click(function(){
        var t = $(this);
        var a = $('.global-page a');
        a.removeClass('select');
        t.addClass('select');
        var index = t.index(a);
        $('.district-list').removeClass('current').eq(index).addClass('current');
    });
}

function interestBox(){
	$('.interest-nav .interest-tab').click(function(){
		var t = $(this);
		var tab = $('#interestBox .interest-nav .interest-tab');
		var index = tab.index(t);
		var tag = $('#interestBox .interest-tags .interest-tag-list');
		var crr = tag.eq(index);
	...
	$('.interest-tags .interest-tag').click(function(){
		var t = $(this);
		var lid = t.attr('lid');
		$.getJSON(APP+'/Member/doInterest/id/'+lid,function(data){
			if(data.status == 1){
				if(t.is('.selected')){
					t.removeClass('selected');
				}else{
...
function memberIndex(item){
	memberTop();
	$('.mo-user-list li').live('mouseover',function(){
		$(this).find('.close').show();
	}).live('mouseout',function(){
		$(this).find('.close').hide();
	});
	
	$('.mo-user-list li .close').live('click',function(){
		var t = $(this).parent();
		$.getJSON(APP+'/Circle/randomDaren/num/1',function(data){
			if(data.status == 1){
				var cs = $(data.info);
				cs.insertBefore(t);
				cs.hide();
	...
	$('.daren .add-attention').live('click',function(){
		var t = $(this);
		var uid = t.attr('uid');
		$.getJSON(APP+'/Member/attention/id/'+uid,function(data){
			if(data.status == 1){
				var close = t.siblings('.close');
				close.trigger('click');
			}else{
				t.tip(data.info,3000);
			}
	...
	$('.other-block .switch').click(function(){
		$('.mo-user-list').hide();
		$('.block-loading').show();
		$.getJSON(APP+'/Circle/randomDaren/num/3',function(data){
			if(data.status == 1){
				$('.block-loading').hide();
				$('.mo-user-list').html(data.info).show();
			}else{
				t.tip(data.info,2000);
			}
		});
	...
	function reset(){
		var href = $('.myhome-nav .tab').eq(1).attr('href');
		var $container = $('#main .column-wraper');
		var $page = $('#split_page');
		loading.loadingStart('#main');
		$.post(href,function(data){
...
function memberEdit(){
	...
	$('#confirmPhoneNo').click(function(){
		jvfDialog('phone_bind', APP+'/Member/verifyPhone');
	});
...
function memberPayment(){
	$('#payForm').submit(function(){
		...
		$.post(APP+'/Member/pay',para,function(data){
			if(data.status == 0){
				...
				var id = guid();
				jvfDialogHtml(id,data.info.html);
			}
		},'json');
		return false;
	});
	...
	//计算金额
	var countPrice = function(index,param){
		...
		var total=Number($('#totalPrices').html());
		var incharge=Number($('#incharge').html());
		var myMoney=Number($('#balanceTf').html());
		if(myMoney<total-incharge){
			$('#needPriceWrap').show();
			$('#showBanks').show();
			$('#needPrice').html(toPrice(Number(total-incharge-myMoney).toFixed(2)));
		}else{
			$('#needPriceWrap').show();
			$('#showBanks').show();
			$('#needPrice').html(toPrice(Number(0).toFixed(2)));
		}
	}(0,'none');
...
function memberVerifycoupon(){
	...
	$("#coupon-query").click(function(){
		...
		var url = APP+'/Company/querycoupon/';
...
function userNoverifymail(){
	...
	$("#aModifyEMail").click(function(){
		$('#change_mail_content').dialog({
			modal: true,
			resizable:false,
			width:600
		});
...
function ajaxCircle(){
	$('.jvf_pubu').live('mouseover',function(){
		$(this).find('.operate-btns').show();
	}).live('mouseout',function(){
		$(this).find('.operate-btns').hide();
	});
	
	$('.jvf_pubu .operate-btns .like-btn').live('click',function(){
		var t = $(this);
		var url = t.attr('href');
		$.getJSON(url,function(data){
			//t.tip(data.info,2000);
			if(data.status==1){
				var like = t.parents('.jvf_pubu').find('.data-block .like-num');
				if(t.is('.btn-love')){
					t.removeClass('btn-love').addClass('btn-loved');
					like.text(parseInt(like.html())+1);
				}else{
					t.removeClass('btn-loved').addClass('btn-love');
					like.text(parseInt(like.html())-1);
				}
			}
		});
	...
	$('.jvf_pubu .explain-wrap').live('mouseover',function(){
		var explain = $(this).find('.explain');
		explain.stop();
			explain.animate({height: '44px'}, 200);
	}).live('mouseout',function(){
		var explain = $(this).find('.explain');
		explain.stop();
			explain.animate({height: '0px'}, 200);
	});
	
	$('.jvf_pubu .operate-menu-share').live('mouseover',function(){
		$(this).find('.share-operate').show();
	}).live('mouseout',function(){
		$(this).find('.share-operate').hide();
	});
	...
	$('.share-operate .trans-btn').live('click',function(){
		var tid = $(this).attr('tid');
		jvfDialog(guid(), APP+'/Member/forwarding/tid/'+tid);
	});
	
	$('.operate-btns .btn-comment').live('click',function(){
		var jvf_pubu = $(this).parents('.jvf_pubu');
		var tid = $(this).attr('tid');
		var mini_comment = jvf_pubu.find('.mini-comment');
		var loading = mini_comment.find('.comm-loading');
		loading.siblings().remove();
		var $container = jvf_pubu.parents('.masonry');
		if(mini_comment.is(':hidden')){
			mini_comment.show();
			$.get(APP+'/Talk_about/getComment/tid/'+tid,function(data){
				loading.hide();
		...
		jvf_pubu.find('.submit .btn-faces').qqFace('.comment_form',function(t){
			commentNum(t);
	...
	$('.jvf_pubu textarea[name="content"]').live('keyup',function(e){
		var t = $(this);
		commentNum(t);
	});
	
	$('.jvf_pubu .submit .panel-checkbox').live('click',function(){
		var t = $(this);
	...
	$('.jvf_pubu .submit .comment-btn').live('click',function(){
		var t = $(this);
		var jvf_pubu = t.parents('.jvf_pubu');
		var form = t.parents('.comment_form');
		var para = form.serialize();
		if(t.is('.btn-t6')){
			$.post(APP+'/Member/doTalk_about_comment',para,function(data){
				if(data.status == 1){
					$.get(APP+'/Talk_about/getCommentOne/id/'+data.info,function(data){
						jvf_pubu.find('.comment-list ul').prepend(data);
						var $container = jvf_pubu.parents('.masonry');
						$container.masonry('reload');
					});
					var content = form.find('textarea[name="content"]');
	...
	$('.jvf_pubu .comment-list .del').live('click',function(){
		var t = $(this);
		var li = t.parents('li');
		var cid = t.attr('cid');
		$.getJSON(APP+'/Member/comment_del/id/'+cid,function(data){
			if(data.status == 1){
				li.fadeOut(function(){
					var $container = li.parents('.masonry');
					li.remove();
					$container.masonry('reload');
				});
			}else{
...
function forwarding(){
	$('#comment_check,#orign_check,#profile_check').click(function(){
		var t = $(this);
		var id = t.attr('id');
		if(t.is('.checked')){
			t.removeClass('checked').addClass('checkbox');
			t.siblings('input[name="'+id+'"]').val(0);
		}else{
			t.removeClass('checkbox').addClass('checked');
			t.siblings('input[name="'+id+'"]').val(1);
		}
	});
	var totalNum = function(){
		var num = getLength($('#jvf_forwarding textarea[name="content"]').val());
		var textNum = 140 - num;
		return textNum;
	}
	
	var countNum = function(){
		var textNum = totalNum();
		if(textNum < 0){
			html = '<div class="count-txt error">超出<span class="txt">'+Math.abs(textNum)+'</span>字</div>';
		}else{
			html = '<div class="count-txt">还可以输入<span class="txt">'+textNum+'</span>字</div>';
		}
		$('#jvf_forwarding #textNum').html(html);
	}
	$('#jvf_forwarding textarea[name="content"]').keyup(function(e){
		countNum();
	});
	
	$('#jvf_forwarding #trans_confirm').click(function(){
		var t = $(this);
		var form = $('#jvf_forwarding');
		var textNum = totalNum();
		if(textNum < 0){
			t.tip('不能超过140字',3000);
			return false;
		...
		var para = form.serialize();
		$.post(APP+'/Member/doTalk_about_tabroadcast',para,function(data){
			if(data.status == 1){
				var ui_diglog = t.parents('.ui-dialog');
				ui_diglog.find('.ui-dialog-titlebar-close').trigger('click');
				ui_diglog.remove();
				jalert('转发成功');
			}else{
				t.tip(data.info,3000);
...
function jalert(msg,opt){
    ...
    op = $.extend(option,op);
	var uuid = guid();
	var html = '<div class="panel-mb-tips" title="'+op.title+'" style="width:300px;height:140px;"><div class="panel-content"><div class="bd"><div class="bb-confirm-'+op.status+'"><h4>'+msg+'</h4><div></div></div></div><div class="ft"></div></div><span class="sd"></span></div>';
	jvfDialogHtml(uuid,html);
	setTimeout(function() {
		var ui_diglog = $('#'+uuid).parents('.ui-dialog');
		ui_diglog.find('.ui-dialog-titlebar-close').trigger('click');
		ui_diglog.remove();
        if(op.callback){
            op.callback();
...
function circleApp(){
	$('.section .switch-nav li').click(function(){
		var t = $(this);
		var lis= t.parents('.switch-nav').find('li');
		var index = lis.index(t);
		var content = t.parents('.section').find('.switch-content .container');
		var crr = content.eq(index);
		lis.removeClass('selected');
		content.removeClass('selected');
		t.addClass('selected');
		crr.addClass('selected');
	});
}

function circlePhilosopher(){
	$('.follow-checkbox').click(function(){
		var t = $(this);
		var all = t.parents('.show-block').find('.check-all');
		var follow = t.parents('.show-block').find('.follow-checkbox');
		if(t.is('.checked')){
			t.removeClass('checked').addClass('checkbox');
			all.removeClass('checked').addClass('checkbox');
		}else{
			t.removeClass('checkbox').addClass('checked');
			var bool = true;
			...
			if(bool){
				all.removeClass('checkbox').addClass('checked');
			}
		}
	});
	...
	$('.check-all').click(function(){
		...
		if(t.is('.checked')){
			t.removeClass('checked').addClass('checkbox');
			follow.removeClass('checked').addClass('checkbox');
		}else{
			t.removeClass('checkbox').addClass('checked');
			follow.removeClass('checkbox').addClass('checked');
		}
	});
		...
		loginAfter(function(){
			$.getJSON(APP+'/Member/attention/id/'+uid,function(data){
				if(data.status == 1){
					...
					t.parents('dl').find('.follow-checkbox').hide();
				}else{
					t.tip(data.info,3000);
				}
			});
	...
	$('.follow-btn').click(function(){
		var t = $(this);
		var follow = t.parents('.show-block').find('.btn-follow-20-add');
		follow.trigger('click');
		t.parents('.show-ft').hide();
	});
	...
	$('.show-nav li').click(function(){
		...
		var show = $('.show-block');
		var index = lis.index(t);
		var crr = show.eq(index);
		lis.removeClass('on');
		t.addClass('on');
		var top = crr.offset().top;
		$('html,body').animate({scrollTop:top});
	});
}

function mallIndex(){
	$('.showindow .cell').hover(function(){
		var t = $(this);
		t.addClass('curr');
		t.find('.pic').addClass('active');
	},function(){
		var t = $(this);
		t.removeClass('curr');
		t.find('.pic').removeClass('active');
	});
