...
class GoodsAction extends CommonAction
{
    ...
    public function index()
    {
    	...
    	$crr = $goods_category->getOne($goodsdata['cid']);
    	$cidarr = explode(',', $crr['path']);
    ...
    public function view(){
    	...
    	if($id){
	    	...
	    	$data = Init_GP(array('title','short_title','cid','category_name','rid','region_name','original','payment','price','deposit','num','onenum','pre','starttime','endtime','tel','address','longitude','latitude','zoom','keywords','description','egid'));
	    	...
	    	$data['promulgator'] = $businessesdata;
    	...
    	$goods_categorydata = $goods_category->getData($goods_categorymap);
    	$crr = $goods_category->getOne($data['cid']);
    	$cidarr = explode(',', $crr['path']);
    	$this->assign('cidarr',$cidarr);
    	$this->assign('goods_categorydata',$goods_categorydata);
    ...
    public function doRelease(){
    	...
    	if($id){
    		...
    		if($goodsdata['promulgator'] != $businessesdata['id']){
    			$this->error(L('operational_error'));
    		}
    	}
    	...
    	if($id){
			...
			$businessesdata = $businesses->getOne($businessesmap);
    		$medata['status'] = 1;
    		$medata['promulgator'] = $businessesdata['id'];
    		//保存当前数据对象
