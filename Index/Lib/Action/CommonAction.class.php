...
class CommonAction extends Action
{
		...
		if(isset($_COOKIE['islogin']) && isset($_COOKIE['mb_id'])){
		    ...
		    if($islogin==1){
				...
				if(!empty($memberdata)){
					...
					$memberdata['like_ids'] = $member->getLikeIds($memberdata['id']);
	...
	protected function _switchRegion(){
		//if(C('sysconfig.is_switch_region')){
			...
			//}
			$re = $region->getChild($arr['id']);
			$arr['child'] = $re[0]['child'];
			$data['crr'] =  $arr;
	...
	protected function _announcement(){
		$article = D('Article');
		$announcement = $article->getCategoryArticle(2,5);
	...
	protected function _footerArticle(){
		...
		$footerArticle = $articles_category->getData($articles_categorymap,3);
	...
	protected function _login_port(){
	...
	protected function _share(){
        ...
        foreach($circledata as &$value){
            $value['share'] = $circle->getLabel($value['id']);
        }

        $this->assign('share',$circledata);
	}

    protected function _circle(){
        $circle = D('Circle');
    ...
    protected function _category(){
        ...
        $categorydata = $goods_category->getData($goods_categorymap);
