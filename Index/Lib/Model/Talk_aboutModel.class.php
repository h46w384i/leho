...
class Talk_aboutModel extends CommonModel {
	...
	public function getDataAll($map,$limit,$order = 'id desc'){
		...
		$sql = "SELECT TA.*,G.title,M.name,M.daren,A.thumbnail as header FROM `{$prefix}talk_about` as TA left join `{$prefix}goods` as G on TA.gid = G.id left join `{$prefix}member` as M on TA.uid = M.id left join `{$prefix}accessory` as A on M.header = A.id {$and} order by {$order} {$limit}";
		$data = $this->query($sql);
	...
	public function getLast($uid){
		...
		$value = $this->where($map)->order('id desc')->find();
		$value['accessory'] = $this->getAccessory($value['id']);
		return $value;
	}
