...
class MemberModel extends CommonModel {
	...
	function getWasLikeNum($uid){
		$talk_about = D('Talk_about');
		$talk_aboutmap = array(
				'uid'=>array('eq',$uid),
		);
		$waslikenum = $talk_about->where($talk_aboutmap)->sum('likes');
		return $waslikenum?$waslikenum:0;
	}
	...
	function getAllNum($uid){
		...
		$data['was_like'] = $this->getWasLikeNum($uid);
	...
	//获取喜欢的集合
	function getLikeIds($uid){
		$talk_about_like = D('Talk_about_like');
		$talk_about_likemap['uid'] = array('eq',$uid);
		return explode(',', $talk_about_like->getGather($talk_about_likemap,'tid'));
	}
	...
	public function randomDaren($num){
		$membermap['daren'] = array('eq',1);
		$uids = array_flip($this->getGatherArr($membermap));
		if(count($uids) > $num){
			$uids = array_rand($uids,$num);
		}
		$map['id'] = array('in',$uids);
		$darendata= $this->getDataAll($map);
		shuffle($darendata);
		return $darendata;
	}
}
