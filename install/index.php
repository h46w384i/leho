...
if($method == 'show_license') {
	...
	if($submit) {
		...
		$time = time();
		$db->query("REPLACE INTO `{$tablepre}user` (`id`, `account`, `nickname`, `password`, `last_login_time`, `last_login_ip`, `login_count`, `status`) VALUES ('1', '{$username}', '管理员', '{$password}', '', '', '0', '1');");
        adminconfig_edit();
	...
	@touch($lockfile);
	@touch(ROOT_PATH.'./Public/upgrade.lock');
	if(VIEW_OFF) {
