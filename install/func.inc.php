...
<meta content="JVFNET Inc." name="Copyright" />
</head>
<div class="container">
	<div class="header">
		<h1>$title</h1>
...
function show_footer($quit = true) {
	...
	echo <<<EOT
		<div class="footer">&copy;2001 - 2010 <a href="http://www.jvfnet.com/">Jvfnet</a> Inc.</div>
	</div>
</div>
</body>
</html>
...
function config_edit() {
    ...
    $sitekey = generate_key();
	$configfile = @file_get_contents(CONFIG);
	$configfile = trim($configfile);
	$configfile = preg_replace("/'DB_HOST'=>'(.*?)'/is", "'DB_HOST'=>'{$dbhost}'", $configfile);
	$configfile = preg_replace("/'DB_NAME'=>'(.*?)'/is", "'DB_NAME'=>'{$dbname}'", $configfile);
	$configfile = preg_replace("/'DB_USER'=>'(.*?)'/is", "'DB_USER'=>'{$dbuser}'", $configfile);
	$configfile = preg_replace("/'DB_PWD'=>'(.*?)'/is", "'DB_PWD'=>'{$dbpw}'", $configfile);
	$configfile = preg_replace("/'DB_PREFIX'=>'(.*?)'/is", "'DB_PREFIX'=>'{$tablepre}'", $configfile);
	$configfile = preg_replace("/'DB_PORT'=>'(.*?)'/is", "'DB_PORT'=>'{$dbport}'", $configfile);
    $configfile = preg_replace("/'SITE_KEY'=>'(.*?)'/is", "'SITE_KEY'=>'{$sitekey}'", $configfile);
	@file_put_contents(CONFIG, $configfile);
}
function adminconfig_edit() {
	extract($GLOBALS, EXTR_SKIP);
	$configfile = @file_get_contents(ADMINCONFIG);
	$configfile = trim($configfile);
	$configfile = preg_replace("/'DEFAULT_ADMIN'=>'(.*?)'/is", "'DEFAULT_ADMIN'=>'{$username}'", $configfile);
	@file_put_contents(ADMINCONFIG, $configfile);
}
